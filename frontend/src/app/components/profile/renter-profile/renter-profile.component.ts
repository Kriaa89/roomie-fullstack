...
import { RenterProfile, RenterProfileUpdateRequest, Skill, LifestylePreference } from '../../../models/profile.model';
...
export class RenterProfileComponent implements OnInit {
  ...
  constructor(
    ...
    this.profileForm = this.formBuilder.group({
      ...
      profilePicture: [''],
      skillsExpected: [[], Validators.required],
  ...
  private loadProfile(userId: number): void {
    this.profileService.getRenterProfile(userId).subscribe({
      next: (profile) => {
        ...
        this.profileForm.patchValue({
          ...
          skillsExpected: profile.skillsExpected,
  ...
  toggleEditMode(): void {
    ...
    if (!this.editMode && this.profile) {
      ...
      this.profileForm.patchValue({
        ...
        skillsExpected: this.profile.skillsExpected,
  ...
  onSubmit(): void {
    ...
    const profileData: RenterProfileUpdateRequest = {
      bio: this.profileForm.value.bio,
      city: this.profileForm.value.city,
      profilePicture: this.profileForm.value.profilePicture,
      skillsExpected: this.profileForm.value.skillsExpected,
    ...
    if (this.profile) {
      ...
      this.profileService.updateRenterProfile(this.currentUser!.id, profileData).subscribe({
        next: (updatedProfile) => {
      ...
      // Create new profile
      this.profileService.createRenterProfile(profileData as any).subscribe({
        next: (newProfile) => {
  ...
  isSkillSelected(skill: Skill): boolean {
    return this.profileForm.value.skillsExpected?.includes(skill) || false;
  }

  toggleSkill(skill: Skill): void {
    const currentSkills = [...(this.profileForm.value.skillsExpected || [])];
    ...
    this.profileForm.patchValue({ skillsExpected: currentSkills });
  }
